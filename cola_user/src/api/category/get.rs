...
pub struct UserCategorytGetApi;

// 构造函数
impl UserCategorytGetApi {
    //
    ...
    ) -> AppData<ListResponse<UserCategoryInfo>> {
        // Call Case ..
        match UserCategoryGetCase::case_get_new_list(uid, url.limit, url.offset, ctx).await {
            Ok(response) => {
                // 打印成功日志
                info!(
