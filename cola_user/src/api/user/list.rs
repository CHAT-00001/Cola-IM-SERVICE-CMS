// cola_user/src/api/user/list.rs
...
impl UserListApi {
    ...
    /// # 2. [API HANDLER] - 热门
    /// * `desc`: 获取热门用户列表
    pub async fn api_get_hot(
        _uid: i64, // 操作者ID
        url: ApiGatewayRequest, // 网关请求
        ctx: &AppContext, // 全局上下文
    ) -> AppData<Vec<UserVo>> {
        match UserListCase::case_get_hot_users(0, url.offset, url.limit, ctx).await {
            Ok(vos) => {
                info!("[🗣️ USER LIST API] - ✅️ 热门用户查询成功");
                AppData::ok(vos)
            }
            Err(e) => {
                error!("[🤐 USER LIST API] - ❌️ 热门用户查询失败: {}", e);
                AppData::err(error::INTERNAL_ERROR, "热门用户查询失败", None)
            }
        }
    }
    ...
    pub async fn api_get_city(
        ...
        match UserListCase::case_get_city_users(0, url.offset, url.limit, ctx).await {
            Ok(vos) => {
    ...
    pub async fn api_get_category(
        ...
        match UserListCase::case_get_category_users(0, url.category_id, url.offset, url.limit, ctx).await {
            Ok(vos) => {
    ...
    pub async fn api_search(
        ...
        let lat = url.lat.unwrap_or(0.0);
        let lng = url.lng.unwrap_or(0.0);
        match UserListCase::case_search_users(0, keyword, lat, lng, url.offset, url.limit, ctx).await {
            Ok(vos) => {
