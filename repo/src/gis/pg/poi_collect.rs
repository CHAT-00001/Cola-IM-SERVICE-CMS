// repo/src/gis/pg/collect.rs  -- 仓储中心 - GIS - pg - 兴趣点 收藏记录
// 2026/7/6
...
impl PoiCollectRepo {
    ...
    pub async fn save_collect_by_gis_id(uid: i64, gis_id: i64, cmd: &PoiCollectCommand) -> Result<u64, sqlx::Error> {
        let pool = pg_pool();
        let now = Utc::now();
    ...
    pub async fn find_collect_ids_by_user_id(user_id: i64, keyword: Option<String>, limit: i64, offset: i64) -> Result<Vec<i64>, sqlx::Error> {
        let pool = pg_pool();
        // 补齐: 增加 is_deleted = 0 的过滤条件
        let mut sql = "SELECT gis_id FROM cola_gis.poi_collect WHERE user_id = $1 AND is_deleted = 0".to_string();
        if let Some(ref kw) = keyword { if !kw.is_empty() { sql.push_str(" AND remark ILIKE $2"); } }
        sql.push_str(" ORDER BY add_time DESC LIMIT $3 OFFSET $4");
        let mut query = sqlx::query_scalar::<_, i64>(&sql);
        query = query.bind(user_id);
        if let Some(ref kw) = keyword { if !kw.is_empty() { query = query.bind(format!("%{}%", kw)); } else { query = query.bind(""); } }
