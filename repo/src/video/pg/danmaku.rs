// repo/src/pg/video/pg/danmaku.rs  -- 仓储 - 短视频 - PG - 弹幕
// 2026/6/8 16:57
...
impl DanmakuRepo {
    ...
    pub async fn find_list_by_uids(
        ...
        // 1. 构建基础 SQL 和参数列表
        let mut sql = format!("SELECT {} FROM video WHERE status = 1", DANMAKU_COLUMNS);
    ...
    pub async fn del_danmaku_by_video_id(
        video_id: i64,
    ) -> Result<u64, sqlx::Error> {  // 返回删除的弹幕数量
    ...
    /// * `danmaku_id`: 弹幕 ID
    /// * ``
    pub async fn user_del_danmaku_by_video_id(
        danmaku_id: i64,
    ) -> Result<u64, sqlx::Error> {  // 返回删除的弹幕数量
    ...
    /// * params: 视频ID
    /// * 视频被删除时触发：删除该视频下的所有弹幕
    pub async fn update_danmaku_status_by_user_id(
        user_id: i64,
    ) -> Result<u64, sqlx::Error> {  // 返回删除的弹幕数量
    ...
    /// * 返回当前弹幕的最新点赞数
    pub async fn update_like_danmaku(
        user_id: i64,
        danmaku_id: i64,
