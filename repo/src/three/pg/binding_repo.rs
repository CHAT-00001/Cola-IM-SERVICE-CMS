// repo/src/three/pg/binding_repo.rs  -- 仓储 - THREE - 业务绑定 PG
// 2026/6/30

////////

use crate::pg_pool;
use cola_data::three::entity::three_biz_binding::{
    THREE_BIZ_BINDING_COLUMNS, ThreeBizBindingEntity,
};

////////

/// # [REPO] - 业务绑定 仓储（统一表 three_biz_binding）
...
impl BindingRepo {
    ...
    /// 1. #[REPOSITORY] - 插入或更新（UNIQUE (biz_module, biz_type) 冲突则更新）
    pub async fn upsert(
        three_config_id: i64,
        biz_module: &str,
        biz_type: &str,
    ...
    /// 3. #[REPOSITORY] - 按业务模块+类型查询
    pub async fn find_by_biz(
        biz_module: &str,
        biz_type: &str,
    ) -> Result<Option<ThreeBizBindingEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
