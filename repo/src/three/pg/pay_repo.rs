// repo/src/three/pg/pay_repo.rs  -- 仓储 - THREE - 支付配置 PG
// 2026/6/30

////////

use cola_data::three::entity::pay::{ThreePayConfigEntity, THREE_PAY_CONFIG_COLUMNS};
use crate::pg_pool;
...
impl PayRepo {
    ...
    pub async fn insert(
        ...
        let pool = pg_pool();
        let query = format!(
            r#"INSERT INTO three_pay (type_id, vendor_id, name, mch_id, api_key, notify_url, config_json, remark, status)
    ...
    pub async fn update(
        ...
        let pool = pg_pool();
        let query = format!(
            r#"UPDATE three_pay SET type_id=$1, vendor_id=$2, name=$3, mch_id=$4, api_key=$5, notify_url=$6, config_json=$7, remark=$8, status=$9, updated_at=NOW()
    ...
    pub async fn list_by_type(type_id: i64) -> Result<Vec<ThreePayConfigEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM three_pay WHERE type_id = $1 ORDER BY id DESC",
            THREE_PAY_CONFIG_COLUMNS
        );
        sqlx::query_as::<_, ThreePayConfigEntity>(&query)
            .bind(type_id)
    ...
    pub async fn find_by_id(id: i64) -> Result<Option<ThreePayConfigEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM three_pay WHERE id = $1 LIMIT 1",
            THREE_PAY_CONFIG_COLUMNS
        );
        sqlx::query_as::<_, ThreePayConfigEntity>(&query)
            .bind(id)
    ...
    pub async fn update_status(id: i64, status: i16) -> Result<Option<ThreePayConfigEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "UPDATE three_pay SET status=$1, updated_at=NOW() WHERE id=$2 RETURNING {}",
            THREE_PAY_CONFIG_COLUMNS
        );
        sqlx::query_as::<_, ThreePayConfigEntity>(&query)
            .bind(status).bind(id)
    ...
    pub async fn list_all() -> Result<Vec<ThreePayConfigEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM three_pay ORDER BY id DESC",
            THREE_PAY_CONFIG_COLUMNS
        );
        sqlx::query_as::<_, ThreePayConfigEntity>(&query)
            .fetch_all(&pool)
