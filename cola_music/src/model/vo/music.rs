...
pub struct MusicVo {
    pub music: MusicInfo,              // 音乐元信息（内部包含 id, name, actor 等）
...
impl MusicVo {
    ...
    pub fn from_user(user: UserInfo) -> Self {
        ...
        let temporary_music = MusicInfo {
            id: 0,
            uuid: None,
            user_id: Some(user.id as i64),
            actor: Some(user.nickname.clone()), // 作者名称
            name: format!("@{} 创造的原声", user.nickname), // 音乐名称
            name_en: None,
            cover_url: Some(user.avatar_url.clone()), // 临时用作者头像当封面
            duration: 0,
            release_time: chrono::Utc::now().format("%Y-%m").to_string(),
            href: "".to_string(),
