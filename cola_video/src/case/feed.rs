// cola_video/src/case/feed.rs  -- VIDEO - 用例层 - FEED
// 2026/5/23 00:01
...
use repository::video::service::feed::FeedService;
...
impl FeedCase {
    ...
    pub async fn case_feed_recommend(
        ...
        let infos =
            ...
            ViewService::batch_get_videos_infos(ids)
            .await
    ...
    pub async fn case_feed_publish(
        ...
        let infos = ViewService::get_videos_infos_by_uid(url.user_id, Some(url.keyword), url.offset, url.limit)
            .await
            .map_err(|e| {
                anyhow::anyhow!("BIZ: 获取用户 {} 发布的视频列表失败: {}", url.user_id, e)
            })?;
