// cola_fs/src/api/media.rs -- FS - 接口层 - 对象媒体
...
impl MediaApi {
    ...
    pub async fn api_add_media(
        ...
        match FsBucketCase::case_add_bucket(uid, cmd, ctx).await {
            Ok(data) => {
                tracing::info!("[🗣️ API] - ✅️ 创建媒体对象成功: uid={}", uid);
                AppData::ok(data)
            }
            Err(e) => {
                tracing::error!("[🤐 API] - ❌️ 创建媒体对象失败: {}", e);
    ...
    pub async fn api_get_media(
        ...
        match FsBucketCase::case_get_bucket(app_id, ctx).await {
            Ok(data) => {
                tracing::info!("[🗣️ API] - ✅️ 查询媒体对象成功");
                AppData::ok(data)
            }
            Err(e) => {
                tracing::error!("[🤐 API] - ❌️ 查询媒体对象失败: {}", e);
                AppData::err(error::INTERNAL_ERROR, &e.to_string(), None)
            }
        }
    }
