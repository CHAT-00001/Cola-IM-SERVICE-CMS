...
impl CdnCase {
    ...
    pub async fn case_change_cdn_status(
        ...
        info!(
            "[🗣️ CASE] - ✅️ CDN状态更新成功: cdn_id={}, status={}",
            cdn_id, status
        );
    ...
    /// # 5. [CASE] - 按应用查询 Bucket CDN
    /// * `desc`: `从 Bucket Port 读取 BucketInfo.cdn_domain`
    pub async fn case_get_cdn(app_id: String, ctx: &AppContext) -> Result<serde_json::Value> {
        let bucket = ctx
            .fs
            .bucket
            .get
    ...
    /// * `desc`: `管理员视角，从 Bucket 列表读取 cdn_domain 和总数`
