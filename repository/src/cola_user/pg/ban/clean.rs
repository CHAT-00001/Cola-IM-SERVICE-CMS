// repository/src/cola_user/service/ban/clean.rs
// 仓储层 - USER - pg - 封禁 - 清除
// 2026/8/3 19:48 Created.
...
/// # [CLEAN REPOSITORY] - 清理
/// * `desc`: `用户封禁清除仓储`
pub struct UserBanCleanRepo;

// 构造函数
impl UserBanCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user.ban"
            WHERE id IN (
                SELECT id FROM "cola_user.ban"
                WHERE is_deleted = 1
