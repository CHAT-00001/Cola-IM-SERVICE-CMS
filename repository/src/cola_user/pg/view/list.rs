// repository/src/pg/cola_user/pg/view/list.rs
// 仓储 - 可乐用户 - pg - 浏览 - 列表
...
impl UserViewListRepo {
    ...
    pub async fn find_new_list_by_user_id(
        ...
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM \"cola_user\".\"view\" WHERE uid = $1 ORDER BY id DESC LIMIT $2 OFFSET $3",
            USER_VIEW_COLUMNS
        );
    ...
    pub async fn find_new_list_by_video_id(
        ...
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM \"cola_user\".\"view\" WHERE video_id = $1 ORDER BY id DESC LIMIT $2 OFFSET $3",
            USER_VIEW_COLUMNS
        );
