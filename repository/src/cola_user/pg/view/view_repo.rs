// repository/src/pg/cola_user/pg/view/view_repo.rs
// 仓储 - 可乐用户 - pg - 浏览 - 原始
// 2026/6/29 03:54
...
impl UserViewRepo {
    ...
    pub async fn find_user_by_id(
        ...
        let query = format!(
            "SELECT {} FROM \"cola_user\" WHERE id = $1 LIMIT 1",
            USER_COLUMNS
        );
    ...
    pub async fn find_user_by_phone(
        ...
        let pool = pg_pool(); // 👈 抽出来
        let query = format!(
            "SELECT {} FROM \"cola_user\" WHERE phone = $1 LIMIT 1",
            USER_COLUMNS
        );
    ...
    pub async fn find_user_by_email(
        ...
        let pool = pg_pool(); // 👈 抽出来
        let query = format!(
            "SELECT {} FROM \"cola_user\" WHERE email = $1 LIMIT 1",
            USER_COLUMNS
        );
    ...
    pub async fn find_batch_users_by_ids(
        ...
        let pool = pg_pool(); // 👈 抽出来
        let query = format!(
            "SELECT {} FROM \"cola_user\" WHERE id = ANY($1) AND status = 1 ORDER BY id DESC",
            USER_COLUMNS
        );
    ...
    pub async fn find_category(
        ...
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM \"cola_user\" WHERE status = 1 ORDER BY likes DESC NULLS LAST, id DESC LIMIT $1 OFFSET $2",
            USER_COLUMNS
        );
