// repository/src/cola_user/service/profile/clean.rs
// 仓储 - USER - service - profile - 清除
...
/// # [MANAGE REPOSITORY] - 用户 资料 清除 仓储
pub struct UserProfileCleanRepo;

// 构造函数
impl UserProfileCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user"."profile"
            WHERE id IN (
                SELECT id FROM "cola_user"."profile"
                WHERE is_deleted = 1
