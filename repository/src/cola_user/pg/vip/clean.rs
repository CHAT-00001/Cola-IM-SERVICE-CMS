// repository/src/cola_user/service/vip/clean.rs
// 仓储 - USER - service - vip - 清除
...
/// # [MANAGE REPOSITORY] - 用户 贵宾 清除 仓储
pub struct UserVipCleanRepo;

// 构造函数
impl UserVipCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user"."vip"
            WHERE id IN (
                SELECT id FROM "cola_user"."vip"
                WHERE is_deleted = 1
