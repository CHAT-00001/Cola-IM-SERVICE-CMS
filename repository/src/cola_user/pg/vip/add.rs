// repository/src/cola_user/pg/vip/add.rs
// 仓储 - USER - vip - add 添加
...
impl VipAddRepo {
    ...
    /// * `desc`: 在 user_vip 表中插入一条充值记录，同时更新 cola_user 表的 is_vip 状态
    pub async fn pg_save_vip_record(
        ...
        // 同时更新 cola_user 表的 is_vip 状态
        let _ = sqlx::query(
            r#"UPDATE "cola_user" SET is_vip = 1, updated_at = NOW() WHERE id = $1"#,
        )
        .bind(target_id)
        .execute(&pool)
        .await?;
    ...
    pub async fn pg_cancel_vip_record(
        ...
        // 同时取消 cola_user 表的 is_vip
        let _ = sqlx::query(
            r#"UPDATE "cola_user" SET is_vip = 0, updated_at = NOW() WHERE id = $1"#,
        )
        .bind(target_id)
        .execute(&pool)
        .await?;
    ...
    pub async fn pg_check_vip_status(user_id: i64) -> Result<bool, sqlx::Error> {
        ...
        let exists: Option<i64> = sqlx::query_scalar(
            r#"SELECT 1 FROM "cola_user" WHERE id = $1 AND is_vip = 1 LIMIT 1"#,
        )
        .bind(user_id)
        .fetch_optional(&pool)
        .await?;
