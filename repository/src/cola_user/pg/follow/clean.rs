// repository/src/cola_user/service/follow/clean.rs
// 仓储 - USER - service - follow - 清除
...
/// # [MANAGE REPOSITORY] - 用户 关注 清除 仓储
pub struct UserFollowCleanRepo;

// 构造函数
impl UserFollowCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user"."follow"
            WHERE id IN (
                SELECT id FROM "cola_user"."follow"
                WHERE is_deleted = 1
