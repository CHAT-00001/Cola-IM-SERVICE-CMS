// repository/src/cola_user/pg/category/clean.rs
// 仓储 - USER - pg - 分类 - 清除
// 2026/8/3 19:49 Created.
...
/// # [CLEAN REPOSITORY] - 清理
/// * `desc`: `用户分类清除仓储`
pub struct UserCateCleanRepo;

// 构造函数
impl UserCateCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user"."black"
            WHERE id IN (
                SELECT id FROM "cola_user"."black"
                WHERE is_deleted = 1
