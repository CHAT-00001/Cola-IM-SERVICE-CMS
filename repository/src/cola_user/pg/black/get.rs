// repository/src/cola_user/pg/black/get.rs
...
impl UserBlackGetRepo {
    ...
    pub async fn find_black_ids_by_uid(
        ...
        let query = "SELECT black_id FROM \"cola_user.black\" WHERE uid = $1 AND status = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
    ...
    pub async fn find_blacker_ids_by_user_id(
        ...
        // 只查询 uid (拉黑者 ID)，且不进行 JOIN，直接查 cola_user.black 表
        let query = "SELECT uid FROM \"cola_user.black\" WHERE user_id = $1 AND status = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
    ...
    pub async fn save_black_record(
        ...
        let query = r#"
        INSERT INTO "cola_user.black" (uid, user_id,remark, status, created_at)
        VALUES ($1, $2, $3, $4, NOW())
