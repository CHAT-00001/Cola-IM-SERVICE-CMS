// repository/src/video/pg/ban/manage.rs -- 仓储 - VIDEO - PG - 封禁 - 管理仓储
// 仓储 - ▶ 视频 - pg - 封禁 - 管理员 仓储
...
use crate::pg_pool;
use cola_data::cola_video::entity::ban::VideoBanEntity;
...
/// * `desc`: `▶ 视频 - 管理员列表仓储`
pub struct VideoBanManageRepo;

impl VideoBanManageRepo {
    //
    ...
    pub async fn find_admin_dislike_record_list(
        ...
        limit: i64,              // 数量
        offset: i64,             // 页码
    ) -> Result<Vec<VideoBanEntity>, sqlx::Error> {
        let pool = pg_pool();

        // 初始化 SQL 查询构建器，带上基础固定条件
        let mut query_builder: QueryBuilder<Postgres> =
            QueryBuilder::new("SELECT * FROM cola_video.ban WHERE status = ");
        ...
        // 构建目标实体查询
        let query = query_builder.build_query_as::<VideoBanEntity>();

        // 执行查询并加上错误日志打印
        query.fetch_all(&pool).await.map_err(|e| {
            tracing::error!(
                error = ?e,
                "[🤐 PG REPO] - ❌️VideoLikeManageRepo::find_admin_dislike_record_list query failed"
            );
            e
        })
    }
