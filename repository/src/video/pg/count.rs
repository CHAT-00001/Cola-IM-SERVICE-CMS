// repository/src/pg/video//count.rs  -- 存储 - PG - 短视频 - 计数
// 2026/6/10 05:41
...
impl CountRepo {
    ...
    pub async fn pg_update_video_views(video_id: i64) -> Result<(), sqlx::Error> {
        let pool = pg_pool();
        let query = r#"
            INSERT INTO video_count (video_id, views)
            VALUES ($1, 1)
            ON CONFLICT (video_id)
            DO UPDATE SET views = video_count.views + 1
        "#;
    ...
    pub async fn pg_update_video_comments(
        ...
        let query = r#"
        INSERT INTO video_count (video_id, comments)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET comments = GREATEST(0, video_count.comments + $2::INT)
    "#;
    ...
    pub async fn pg_update_video_likes(
        ...
        let query = r#"
        INSERT INTO video_count (video_id, likes)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET likes = GREATEST(0, video_count.likes + $2::INT)
    "#;
    ...
    pub async fn pg_update_video_unlikes(
        ...
        let query = r#"
        INSERT INTO video_count (video_id, unlikes)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET likes = GREATEST(0, video_count.unlikes + $2::INT)
    "#;
    ...
    pub async fn pg_update_video_collects(
        ...
        let query = r#"
        INSERT INTO video_count (video_id, collects)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET collects = GREATEST(0, video_count.collects + $2::INT)
    "#;
    ...
    pub async fn pg_update_video_recommend(
        ...
        let query = r#"
        INSERT INTO video_count (video_id, recommend)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET recommend = GREATEST(0, video_count.recommend + $2::INT)
    "#;
    ...
    pub async fn pg_update_video_shares(video_id: i64) -> Result<(), sqlx::Error> {
        let pool = pg_pool();
        let query = r#"
            INSERT INTO video_count (video_id, shares)
            VALUES ($1, 1)
            ON CONFLICT (video_id)
            DO UPDATE SET shares = video_count.shares + 1
        "#;
        sqlx::query(query).bind(video_id).execute(&pool).await?;
        Ok(())
    }
