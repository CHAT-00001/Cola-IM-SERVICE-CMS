// repository/src/video/pg/buy/manage.rs -- 仓储 - VIDEO - PG - 购买 - 管理仓储
...
/// [MANAGE REPOSITORY] - 视频购买管理仓储
/// * `desc`: VIDEO - Buy Manage Repository.`
...
impl VideoBuyManageRepo {
    ...
    pub async fn find_admin_list(
        ...
        // 构建目标实体查询
        let query = query_builder.build_query_as::<VideoBuyEntity>();

        // 执行查询并加上错误日志打印
        query.fetch_all(&pool).await.map_err(|e| {
            tracing::error!(
                error = ?e,
                "[🤐 PG REPO] - ❌️ VideoLikeManageRepo::find_admin_dislike_record_list query failed"
            );
            e
        })
    }
