// repository/src/cola_video/pg/share/manage.rs
// 仓储 - ▶ 视频 - pg - 分享- 管理员 仓储
...
/// * `desc`: `▶ 视频 - 管理员列表仓储`
pub struct VideoShareManageRepo;

impl VideoShareManageRepo {
    //

    ////////

    /// # 9. [REPOSITORY] - 管理员列表
    pub async fn find_admin_share_record_list(
        ...
        let mut query_builder: QueryBuilder<Postgres> =
            QueryBuilder::new("SELECT * FROM cola_video.share WHERE status = ");
        ...
        query.fetch_all(&pool).await.map_err(|e| {
            tracing::error!(
                error = ?e,
                "VideoShareManageRepo::find_admin_share_record_list query failed"
            );
            e
        })
    }
