// repository/src/video/pg/video/home
// 仓储 - VIDEO - pg - video - home 主页仓储
...
impl VideoRepo {
    ...
    /// # 1. [REPOSITORY] - 🎥 最新
    pub async fn pg_find_new_list(limit: i64, offset: i64) -> Result<Vec<VideoEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
    ...
    pub async fn pg_find_category_list(
        category_id: i16,
        limit: i64,
        offset: i64,
    ) -> Result<Vec<VideoEntity>, sqlx::Error> {
    ...
    /// # 6. [REPOSITORY] - 🎥 ⭐ 精选
    pub async fn pg_find_featured_list(
        limit: i64,
        offset: i64,
    ) -> Result<Vec<VideoEntity>, sqlx::Error> {
        let pool = pg_pool();
