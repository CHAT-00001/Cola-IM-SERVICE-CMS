// repository/src/video/pg/video/count.rs -- 仓储 - VIDEO - PG - 视频 - 计数仓储
// 2026/6/10 05:41
...
impl VideoCountRepo {
    ...
    pub async fn pg_update_video_recommend(
        ...
        let query = r#"
        INSERT INTO cola_video.video_count (video_id, hotlist)
        VALUES ($1, GREATEST(0, $2::INT))
        ON CONFLICT (video_id)
        DO UPDATE SET hotlist = GREATEST(0, cola_video.video_count.hotlist + $2::INT)
    "#;
