// repository/src/video/pg/video/manage.rs -- 仓储 - VIDEO - PG - 视频 - 管理仓储
...
/// * `desc`: `▶ 视频 - 管理员列表仓储`
pub struct VideoManageRepo;

impl VideoManageRepo {
    //
    ...
    pub async fn find_admin_dislike_record_list(
        ...
        let mut query_builder: QueryBuilder<Postgres> =
            QueryBuilder::new("SELECT * FROM cola_video.video WHERE status = ");
        ...
        // 构建目标实体查询
        let query = query_builder.build_query_as::<VideoEntity>();

        // 执行查询并加上错误日志打印
        query.fetch_all(&pool).await.map_err(|e| {
            tracing::error!(
                error = ?e,
                "[🤐 PG REPO] - ❌️ VideoManageRepo::find_admin_dislike_record_list query failed"
            );
            e
        })
    }
