// repository/src/video/service/pg/perm_check.rs  -- 仓储 - video - PG - 权限检查
// 2026/6/9 11:20
...
impl VideoPermCheckRepo {
    ...
    pub async fn check_video_visibility_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT visibility_perm FROM "video" WHERE id = $1 LIMIT 1"#;
    ...
    pub async fn check_video_comment_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT comment_perm FROM "video" WHERE id = $1 LIMIT 1"#;
    ...
    pub async fn check_video_danmaku_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT danmaku_perm FROM "video" WHERE id = $1 LIMIT 1"#;
    ...
    pub async fn check_video_collect_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT collect_perm FROM "video" WHERE id = $1 LIMIT 1"#;
    ...
    pub async fn check_video_download_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT download_perm FROM "video" WHERE id = $1 LIMIT 1"#;
    ...
    pub async fn check_video_buy_perm(video_id: i64) -> Result<i16, sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"SELECT buy_perm FROM "video" WHERE id = $1 LIMIT 1"#;
