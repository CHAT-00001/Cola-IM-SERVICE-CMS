...
impl VisitedCache {
    ...
    // 1. 记录用户看过的视频
    // =========================
    pub async fn add_visited(&self, user_id: i64, video_id: i64, ts: i64) -> anyhow::Result<()> {
        let mut conn = self.db.redis_conn.clone();
        let key = Self::key(user_id);

        // ⭐ 关键修复：score 必须显式转 f64（Redis ZSET 标准）
        let _: () = conn.zadd(key, video_id, ts as f64).await?;
    ...
    pub async fn get_user_video_ids(
        ...
        // ✔ 标准：按时间倒序
        let ids: Vec<i64> = conn.zrevrange(key, start as isize, end as isize).await?;
    ...
    // 3. 删除某个视频记录
    // =========================
    pub async fn remove(&self, user_id: i64, video_id: i64) -> anyhow::Result<()> {
        let mut conn = self.db.redis_conn.clone();
        let key = Self::key(user_id);
