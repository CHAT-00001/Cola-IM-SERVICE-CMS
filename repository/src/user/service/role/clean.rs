// repository/src/user/service/role/clean.rs
// 仓储 - USER - service - role - 清除
// 2026/8/3 19:27 Created.

////////

use crate::pg_pool;
use crate::user::pg::role::clean::UserRoleCleanRepo;
use cola_data::user::entity::role::UserRoleEntity;
////////

/// # [GET SERVICE] - 用户 角色 管理 服务
pub struct UserRoleCleanService;

// 构造函数
impl UserRoleCleanService {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_user"."role"
            WHERE id IN (
                SELECT id FROM "cola_user"."role"
                WHERE is_deleted = 1
