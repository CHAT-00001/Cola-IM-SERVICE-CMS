...
impl PgProfileListRepo {
    ...
    pub async fn find_new_user_list(
        ...
        let query = format!(
            "SELECT {} FROM \"cola_user.profile\" WHERE status = 1 ORDER BY create_time DESC LIMIT $1 OFFSET $2",
            USER_COLUMNS
        );
    ...
    pub async fn find_user_by_id(user_id: i64) -> Result<Option<UserEntity>, sqlx::Error> {
        ...
        let query = format!(
            "SELECT {} FROM \"cola_user.profile\" WHERE id = $1 LIMIT 1",
            USER_COLUMNS
        );
    ...
    pub async fn find_many_users_by_ids(user_ids: &[i64]) -> Result<Vec<UserEntity>, sqlx::Error> {
        ...
        let query = format!(
            "SELECT {} FROM \"cola_user.profile\" WHERE id = ANY($1) AND status = 1 ORDER BY create_time DESC",
            USER_COLUMNS
        );
    ...
    pub async fn find_nearby_user_list(
        ...
        let pool = pg_pool(); // 👈 抽出来
        // 💡 修正：表名换回 \"cola_user.profile\"
        let query = format!(
            "SELECT {}, SQRT(POW(lat - $1, 2) + POW(lng - $2, 2)) AS distance
             FROM \"cola_user.profile\"
             WHERE status = 1
    ...
    pub async fn find_category(limit: i64, offset: i64) -> Result<Vec<UserEntity>, sqlx::Error> {
        let pool = pg_pool(); // 👈 抽出来
        // 💡 修正：从 \"cola_user.profile\" 表按热度排序（可按你实际的业务权重字段调整）
        let query = format!(
            "SELECT {} FROM \"cola_user.profile\" WHERE status = 1 ORDER BY likes DESC, views DESC LIMIT $1 OFFSET $2",
            USER_COLUMNS
        );
    ...
    pub async fn find_featured(limit: i64, offset: i64) -> Result<Vec<UserEntity>, sqlx::Error> {
        let pool = pg_pool(); // 👈 抽出来
        let query = format!(
            "SELECT {} FROM \"cola_user.profile\" WHERE status = 1 AND is_recommend = 1 ORDER BY likes DESC, create_time DESC LIMIT $1 OFFSET $2",
            USER_COLUMNS
        );
    ...
    pub async fn search_keyword(
        ...
        // 💡 修正：用 \"cola_user.profile\" 表，并将 title 替换为符合 UserEntity 属性的 nickname 模糊查询
        let query = format!(
