...
impl UserRoleAddRepo {
    ...
    pub async fn pg_save_new_role_record(
        ...
        let query = r#"
        INSERT INTO "user.role" (uid, user_id,remark, status, created_at)
        VALUES ($1, $2, $3, $4, NOW())
    ...
    pub async fn find_star_follow_ids(
        ...
        let query = "SELECT follow_id FROM \"user.role\" WHERE uid = $1 AND status = 1 AND star = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
