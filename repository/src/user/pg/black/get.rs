// repository/src/user/pg/black/get.rs
// 仓储层 - 可乐用户 - pg - 黑名单 - 获取仓储
// 2026/8/3 01:16 Created.
...
/// # [GET REPOSITORY] - 获取
/// * `desc`: `仓储层用户黑名单获取仓储`
pub struct UserBlackGetRepo;

// 构造函数
impl UserBlackGetRepo {
    //
    ...
    pub async fn find_black_ids_by_uid(
        ...
        let query = "SELECT black_id FROM \"user.black\" WHERE uid = $1 AND status = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
    ...
    pub async fn find_blacker_ids_by_user_id(
        ...
        // 只查询 uid (拉黑者 ID)，且不进行 JOIN，直接查 user.black 表
        let query = "SELECT uid FROM \"user.black\" WHERE user_id = $1 AND status = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
    ...
    pub async fn find_star_black_ids(
        ...
        // 使用 query_scalar，并指明映射类型为 i64
        let query = "SELECT black_id FROM \"user.black\" WHERE uid = $1 AND status = 1 AND star = 1 ORDER BY id DESC LIMIT $2 OFFSET $3";
