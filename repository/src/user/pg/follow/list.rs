...
impl UserFollowListRepo {
    ...
    pub async fn pg_find_users_by_ids(
        ids: &[i64],
    ) -> Result<Vec<cola_data::user::entity::user::UserEntity>, sqlx::Error> {
        use cola_data::user::entity::user::USER_COLUMNS;
        if ids.is_empty() {
            return Ok(vec![]);
        }
        let pool = pg_pool();
        ...
        sqlx::query_as::<_, cola_data::user::entity::user::UserEntity>(&query)
            .bind(ids)
            .fetch_all(&pool)
            .await
    }
