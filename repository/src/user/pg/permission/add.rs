// repository/src/cola_video/pg/permission/add.rs
// 仓储 - VIDEO - 权限 - 发布
// 2026/8/11 11:13 Created.

////////

use crate::pg_pool;

////////

/// # [ADD REPOSITORY] - 用户权限仓储
pub struct VideoPermissionAddRepo;

impl VideoPermissionAddRepo {
    /// # 1. [REPOSITORY] - 更新用户权限/发布计数
    pub async fn update_user_permission(uid: i64, delta: i32) -> Result<(), sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"UPDATE cola_gis.gis_user SET publish_count = GREATEST(0, publish_count + $2), updated_at = NOW() WHERE uid = $1"#;
        sqlx::query(sql)
            .bind(uid)
            .bind(delta)
            .execute(&pool)
            .await?;
        Ok(())
    }

    /// # 2. [REPOSITORY] - 更新视频评论权限
    pub async fn update_video_comment_perm(
        uid: i64,
        video_id: i64,
        comment_perm: i16,
    ) -> Result<(), sqlx::Error> {
        let pool = pg_pool();
        let sql = r#"UPDATE cola_gis.cola_gis SET comment_perm = $3, updated_at = NOW() WHERE id = $2 AND uid = $1"#;
        sqlx::query(sql)
            .bind(uid)
            .bind(video_id)
            .bind(comment_perm)
            .execute(&pool)
            .await?;
        Ok(())
    }
}

//////// END