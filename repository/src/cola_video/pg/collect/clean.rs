// repository/src/cola_video/pg/collect/clean.rs
// 仓储 - VIDEO - pg - collect - 清除
...
/// # [MANAGE REPOSITORY] - 视频 收藏 清除 仓储
pub struct VideoCollectCleanRepo;

// 构造函数
impl VideoCollectCleanRepo {
    //
    ...
    pub async fn pg_clean_expired_deleted_records() -> Result<u64, sqlx::Error> {
        ...
        let query = r#"
            DELETE FROM "cola_video"."collect"
            WHERE id IN (
                SELECT id FROM "cola_video"."collect"
                WHERE is_deleted = 1
