...
use cola_data::cola_gis::entity::poi::{PoiEntity, GIS_POI_COLUMNS};
use sqlx;
...
impl AddRepository {
    ...
    pub async fn pg_save_gis_by_uid(uid: i64, cmd: PoiCommand, visibility: i16) -> Result<PoiEntity, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
    ...
    pub async fn pg_update_gis_by_id(poi_id: i64, cmd: PoiCommand, visibility: i16) -> Result<PoiEntity, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
        ...
        sqlx::query_as::<_, PoiEntity>(&query)
            .bind(poi_id).bind(cmd.title).bind(cmd.description).bind(cmd.href).bind(visibility)
            .fetch_one(&pool).await
    }
    ...
    pub async fn pg_delete_poi_by_ids(poi_ids: Vec<i64>) -> Result<PoiEntity, sqlx::Error> {
        ...
        sqlx::query_as::<_, PoiEntity>(query).bind(poi_ids).fetch_one(&pool).await
    }
