// repositoryo/src/cola_gis/pg/cola_gis.rs -- 仓储 - GIS - PG - 兴趣点
// 2026/7/6 14:10
...
impl PoiRepo {
    ...
    pub async fn search_keyword_list(
        ...
        if start_time.is_some() {
            sql.push_str(&format!(" AND add_time >= ${}", param_index));
            param_index += 1;
        }
        if end_time.is_some() {
            sql.push_str(&format!(" AND add_time <= ${}", param_index));
    ...
    pub async fn find_list_by_uids(
        ...
        let pool = pg_pool();
        let mut sql = format!(
            "SELECT {} FROM cola_gis.poi WHERE status = 1",
            GIS_POI_COLUMNS
        );
