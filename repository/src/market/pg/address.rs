...
impl AddressRepo {
    ...
    /// # 1. [REPOSITORY] - 插入地址
    pub async fn insert(entity: &AddressEntity) -> Result<AddressEntity, sqlx::Error> {
        let pool = pg_pool();
    ...
    /// # 2. [REPOSITORY] - 更新地址
    pub async fn update(id: i64, entity: &AddressEntity) -> Result<AddressEntity, sqlx::Error> {
        let pool = pg_pool();
        let row = sqlx::query_as::<_, AddressEntity>(
    ...
    /// # 4. [REPOSITORY] - 按ID查地址
    pub async fn find_by_id(uid: i64, id: i64) -> Result<Option<AddressEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
