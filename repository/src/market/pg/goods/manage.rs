...
impl GoodsManageRepo {
    ...
    pub async fn admin_list(
        ...
        // 基础字段过滤
        if let Some(uid) = uid {
            query_builder.push(" AND uid = ").push_bind(uid);
        }
        if let Some(id) = goods_id {
            query_builder.push(" AND id = ").push_bind(id);
        }
        if let Some(c1) = one_classid {
            query_builder.push(" AND one_classid = ").push_bind(c1);
        }
        if let Some(c2) = two_classid {
            query_builder.push(" AND two_classid = ").push_bind(c2);
        }
        if let Some(c3) = three_classid {
            query_builder.push(" AND three_classid = ").push_bind(c3);
        }
        if let Some(city) = city_id {
            query_builder.push(" AND city_id = ").push_bind(city);
        }
        ...
        // 价格区间筛选
        if let Some(lp) = low_price {
            query_builder.push(" AND price >= ").push_bind(lp);
        }
        if let Some(hp) = high_price {
            query_builder.push(" AND price <= ").push_bind(hp);
        }

        // 时间范围筛选
        if let Some(st) = start_time {
            query_builder.push(" AND add_time >= ").push_bind(st);
        }
        if let Some(et) = end_time {
            query_builder.push(" AND add_time <= ").push_bind(et);
        }
