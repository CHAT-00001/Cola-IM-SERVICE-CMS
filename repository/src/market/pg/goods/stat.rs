...
impl GoodsStatRepo {
    ...
    pub async fn count_all_goods() -> Result<u64, sqlx::Error> {
        let pool = pg_pool();
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM market.goods")
            .fetch_one(&pool)
            .await?;
    ...
    pub async fn count_offline_goods() -> Result<u64, sqlx::Error> {
        let pool = pg_pool();
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM market.goods WHERE issale = 0")
            .fetch_one(&pool)
            .await?;
    ...
    pub async fn count_goods_by_uid(uid: i64) -> Result<u64, sqlx::Error> {
        let pool = pg_pool();
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM market.goods WHERE uid = $1")
            .bind(uid)
            .fetch_one(&pool)
            .await?;
