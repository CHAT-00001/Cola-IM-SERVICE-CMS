// repository/src/dynamic/pg/content/add.rs -- 仓储 - DYNAMIC - PG - 动态内容 - 发布仓储
// 2026/9/8 01:42 Created.
...
/// # [ADD REPOSITORY] - 动态内容发布仓储
/// * `desc`: `用户发布视频仓储`
pub struct DynamicContentAddRepository;

// 构造实现
impl DynamicContentAddRepository {
    //
    ...
    /// # 1. [REPOSITORY] - 💾 保存
    /// * `desc`: `根据用户ID - 保存动态记录`
    pub async fn save_dynamic_by_uid(
        uid: i64,
        cmd: VideoNewCommand,
        ...
        let query = format!(
            "INSERT INTO cola_dynamic.content (
    ...
    /// # 2. [REPOSITORY] - 更新动态内容
    /// * `desc`: `根据动态ID - 更新动态记录`
    pub async fn update_content_by_dynamic_id(
        video_id: i64,
