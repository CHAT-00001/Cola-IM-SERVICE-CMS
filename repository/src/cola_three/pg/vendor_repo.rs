// repository/src/cola_three/pg/vendor_repo.rs  -- 仓储 - THREE - 厂商 PG
// 2026/6/30 05:01
...
impl VendorRepo {
    ...
    pub async fn list() -> Result<Vec<ThreeVendorEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM cola_three.vendor ORDER BY sort ASC",
            THREE_VENDOR_COLUMNS
        );
        sqlx::query_as::<_, ThreeVendorEntity>(&query)
            .fetch_all(&pool)
    ...
    pub async fn find_by_id(id: i64) -> Result<Option<ThreeVendorEntity>, sqlx::Error> {
        let pool = pg_pool();
        let query = format!(
            "SELECT {} FROM cola_three.vendor WHERE id = $1 LIMIT 1",
            THREE_VENDOR_COLUMNS
        );
        sqlx::query_as::<_, ThreeVendorEntity>(&query)
            .bind(id)
